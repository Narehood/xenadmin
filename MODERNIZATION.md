...
**Shipped so far (on `development` via PRs #6/#7):** live XenModel connect + preview TOFU, infrastructure tree (pool → host → VM), General summary, public-IP warning (complete IPv4 only), CI artifact `drop-shell-win-x64`.
