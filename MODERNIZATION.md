...
**Shipped so far (on `development`):** live XenModel connect + preview TOFU, infrastructure tree (pool → host → VM), General + Storage + Network + Console (endpoint scaffold) tabs, public-IP warning (complete IPv4 only), CI artifact `drop-shell-win-x64`.

**Next:** interactive Avalonia VNC → General copy-icon affordance → TOFU UX → persistence. Details in `UI_REWRITE.md`.
